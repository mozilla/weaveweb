// This requires strands.js or persevere.js
 /* ***** BEGIN LICENSE BLOCK *****
  ...
  if (kid !== null) {
	...
	//       via the SpiderMonkey-specific __defineGetter__
	 ...
	 var opArity = {
		 ...
		 NEW: 1, NEW_WITH_ARGS: 2, DOT: 2, TRANSIENT_DOT: 2, INDEX: 2, TRANSIENT_INDEX: 2, CALL: 2, YIELDING: 3, YIELD: 1,
...
StrandsCompiler = function (options) {
	...
	if (options.source=='js17')
		options.defaultYield = false;
}
...
strandscp.compile = function (/*string*/ code, /*string*/ scriptName,thisObject) {
	if (!_frm)
		var frame = {_cp:0};
	else
	...
	if (!frame._cp) {
		...
		frame.n = Narcissus.parse(code, scriptName, 1,this.options.expression); 
...
strandscp.treeify = function(n,inFunc) {
	...
	n.treeified = true;
	if (this.options.defaultYield && inFunc && (n.type==Narcissus.CALL || n.type==Narcissus.NEW || n.type==Narcissus.NEW_WITH_ARGS || 
		((n.type==Narcissus.INDEX || n.type==Narcissus.DOT) && (n.parent.type != Narcissus.ASSIGN || n.parent[0] != n) && this.options.getters)))
		n.yielding = 1;
	for (var i in n) {
		if (n[i] != null &&
		    typeof(n[i]) == 'object' &&
			...
			i != "parent") 
		{
			...
			this.treeify(child,inFunc || n.type == Narcissus.FUNCTION);
			if((child.letBlock || child.type==Narcissus.LET) && child.type != Narcissus.BLOCK)
				n.letBlock = true;
		...
		switch(node.type) {
		  ...
		  case FUNCTION:
			...
			if (isYielding(node.body)) {
				...
				var varList = []; 
				for (var j in scopedVars) // suspend operation
					varList.push(j);
				// if there is no inner functions than we can use the faster and easier to debug scope recreation technique
				// if there is an inner function we use a function closure to preserve and reuse the same scope, 
				//however it is harder to debug and slower because it uses the parent scope and it doubles/triples the stack 
				var openNodes = [
					node.hasClosure ? 
					codeNode('var ~=1;var _f=_frm(arguments);return _f.clsr(function(){',varList.push('_cp') && varList.join(',')):
					codeNode('var _f = _frm(arguments);var _cp=_f.scp;'),
				    codeNode('_s:while(1){'),
				    (!options.exceptions ?
						codeNode("$_noex=1;") :
				        node.hasTry ?
				          codeNode('try{') :
				          codeNode('')),
				    codeNode('switch(_cp){case 1:' + (options.source=='js17' && !node.coroutine ? '_cp=2;return _gen(_f);case 2:' : ''))
				];
				while (openNodes.length)
					node.body.unshift(openNodes.pop());
				if (node.hasClosure)
					node.body.push(codeNode('return;' + ((options.exceptions && node.hasTry)?
					"}}catch(ex){_cp=_f.exc(ex)":'') + '}}return _f.sus(this)}).call(this)'));
				else {
					for (var i = 0; i < node.params.length; i++)  {
						scopeResolver.addSymbol(node.params[i].value,node.params[i].varType||ANY_TYPE);
						varList.push(node.params[i] = node.params[i].value);
					}
					node.body.push(codeNode('return;case 0:'));
					for (i = 0; i < varList.length;i++)
						node.body.push(codeNode('~=_f[~];',varList[i],i));
					node.body.push(codeNode('_cp=_f.cp;'));// resume operation
					if (options.exceptions && node.hasTry)
						node.body.push(codeNode("}}catch(ex){_cp=_f.exc(ex)"));
					node.body.push(codeNode("}}"));
					i = 0;
					varList.push('_cp');
					varList.push('this');
					node.body.push(codeNode('return _f.sus(~)',varList.join(',')));
				}
			...
			break;
  		  case ASSIGN:
		  	if (node[0].type==ARRAY_INIT || node[0].type==OBJECT_INIT) {
				var value = node[0];
				ensureFrame(node);
				node[0] = (codeNode("_f.dst"));
				statements.push(node);
				statements.push(codeNode(";"));
				for (var i =0; i < value.length;i++)
					statements.push(codeNode("~=_f.dst[~];",value[i].value,i));
				replaceNode(codeNode("_f.dst"));
			} // fall through
		  case DELETE:
	  		var propName = node[0].value;
		  	if (options.setters && (node[0].type == DOT || node[0].type == INDEX)) {
		  		var index = node[0].type == DOT ? codeNode('"' + propName + '"') : node[0][1];
		  		if (node.type == DELETE)
		  			node[1] = noderize({
			  					type: LIST,
					  			0: node[0][0],
		  	...
		  	break;
		  case YIELD:
		  	if (options.source=='js17' && node.type == YIELD) {
				if (getParentFunction(node) && !getParentFunction(node).coroutine) {
					codePtrSequence++;
					...
					addCode('_cp=~;',
			  		...
			  		node.type = RETURN;
					node.expression = node[0];
					statements.push(node);
					addCode('case ~:',codePtrSequence);
					var newNode = codeNode("_receive");
					newNode.varType = varType;
					replaceNode(newNode);
				}
				else {
					node[0].yielding = true;
					replaceNode(execute(node[0]));					
				}
				break;
			}
		  case NEW: case NEW_WITH_ARGS: case CALL:
			...
			if (node[0].value == "strand" && node[0].type == IDENTIFIER) { 
				node[1][0].coroutine = true;
				replaceNode(execute(node[1][0]));
				break;
			}
			...
			if (!isYielding(node) || !withinYielding(node) || ((node.type==NEW || node.type==NEW_WITH_ARGS) && node[0].value=='Date')) // new can't handle Date
			{
				node[1] = execute(node[1]);
				break;
			}
...
/*			if (!isYielding(node.body)) { // we will always do it so they we can weed out dont-enums.
			...
			statements.push(noderize({
				type: NJS_CODE, 
				value: subst("if((_f.iter~=_f.keys(", iterId),
			...
			node.type = FOR;
			node.setup = codeNode("_f.ctr~=0;", iterId);
			node.condition = codeNode("_f.ctr~<_f.iter~.length",
				                      iterId, iterId);
			node.update = codeNode("_f.ctr~++", iterId);
			var initializer = codeNode("_f.iter~[_f.ctr~]",
				                        iterId, iterId);
		  ...
		  case FOR:
			...
			if(node.setup && node.setup.type != VAR  && node.setup.type != NJS_CODE)
		  ...
		  case TRY:
			...
			for (var i = 0; i < node.catchClauses.length; i++) {
				...
				addCode("~ = _f.ex;", scopeResolver.getSymbol(clause.varName,clause.lineno));
				...
				if (clause.guard) {
					...
					if (i < node.catchClauses.length - 1) {
						...
						addCode("throw ~;", scopeResolver.getSymbol(clause.varName,clause.lineno));
					}
		  ...
		  case IDENTIFIER:
			node.value = scopeResolver.getSymbol(node.value,node.lineno);
		  ...
		  case OR: case AND:
			...
			var condVar = "_f.c" + codePtrSequence;
			...
			node.value = condVar;
		  	break;
		  	case LET:
		  case CONST: node.type = VAR;// IE doesn't support consts, so we will just treat it like a var
		  case VAR:
		  	if(scopeResolver.isYielding() && getParentFunction(node).hasClosure)
		  		node.scoped = true;
  			for (var i = 0; i < node.length; i++) {
				node[i] = execute(node[i]);
			}
		  ...
		  case IS:
		  	...
		  	break;
		  case INDEX: 
		  	if (options.getters && isYielding(node) && node.parent.type != CALL) {
			  	var get = noderize({
							type: CALL,
							parent: node.parent,
							yielding: true,
		  ...
		  case TRANSIENT_INDEX:
				...
				break;
		  case DOT:
			if (options.getters && isYielding(node) && node.parent.type != CALL) {			// don't execute n[1] because it might resolve to a scoped var		
			  	var get = noderize({
							type: CALL,
							parent: node.parent,
							yielding: true,
...
//		  		if (!options.getters)
	...
	function withinYielding(node) {
		return getParentFunction(node).yielding;
	}
	function ensureFrame(node) {
		node = getParentFunction(node);
		if (!node.yielding && !node.hasFrame) {
			node.hasFrame =1;
			statements.push(codeNode('var _f={};'));
		}
	}		
	...
	function addError(message,lineno) {
...
nsrp.addError = function(message,lineno) {
	strandscp.currentError = strandscp.currentError.next ={message:message, lineNumber:lineno || 1};
}
...
nsrp.getSymbol = function(name,lineno) {
	...
	if (!this.globalChecking || window[name] || name=='arguments')
		return name;
	for (var i = this.scopes.length; i > 0;) {
		i--;
		if (this.scopes[i][name]) 
	...
	//this.addError("The identifier " + name + " was not found",lineno); // TODO: Reenable this check at some point	
	return name;
}
...
strandsswp.write = function(n, text) {
	...
	var lineno = n && n.lineno >= this.lines.length ? n.lineno : (this.lines.length || 2) - 1;
...
strandsswp.write_in = function(n) {
	this.add(n[0]);
	this.write(n, " in ");
	this.add(n[1]);
}
...
strands.compiler = new StrandsCompiler({exceptions: true, getters:false, compress : false});
...
strands.compiler.Function = function(source,thisObject,runAt) {
	with(_frm(this,arguments,['source','thisObject','runAt'],[])) {
		if (!source)
			source = "function() {\n}";
		 var code = this.compile("temp=" + source, "input",thisObject);
		 if (code == _S) return _s();	 
		var func = eval(code);
		if (runAt == "server") {
			func = function() {
				return persevere.rpc(this,arguments);
			}
		}
...
strands.compiler.tryCatchCompile = function(source,name,getters,setters,debug,expression,js17) {
	try {
		this.options.getters = getters;
		this.options.setters = setters;
		this.options.debug = debug;
		this.options.expression = expression;
		this.options.source = js17?"js17":"js15";
		this.options.defaultYield = !js17;
