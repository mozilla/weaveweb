...
Weave.Crypto.PKCS12 = function() {
  ...
  /* Convert a Hex SHA1 Digest to string */
  function _digestToString(hex) {
    var ret = '';
    for (var i = 0; i < hex.length; i += 2) {
      ret += String.fromCharCode(parseInt(hex[i] + hex[i+1], 16));
    }
  ...
  function _pkcs12Generator(type, iter, salt, pass) {
    ...
    for (i = 0; i < c; i++) {
      var cHash = D + I;
      for (j = 0; j < iter; j++) {
        cHash = WeaveClient.SHA1.digest(cHash);
      }
      A[i] = _digestToString(cHash);
      B = _replicate(A[i], v);
